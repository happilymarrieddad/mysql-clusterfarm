...
Cluster.prototype.query = function(sql,values,cb,options) {
	...
	vm.getAvailableMaster(function(index) {
		try {
			if (index > -1) {
				if (type == 1) {
					vm.masters[index].query(qry,[],callback,opts)
				} else {
					vm.masters[index].query(qry,values,callback,opts)
				}
			} else {
			...
			try {
				if (type == 1) {
					vm.masters[0].query(qry,[],callback,opts)
				} else {
					vm.masters[0].query(qry,values,callback,opts)
				}
			}catch(err2) {
				callback('No active masters... Please ensure sql masters are live',null)
			}
