...
function PoolClusterFarm(config) {
  ...
  this.useDeadlockHandling = config.useDeadlockHandling || false
  this.deadlockConfig = {
  	retries:5,
  	minMillis:1,
  	maxMillis:100
  }
  if (config.deadlockConfig) {
  	for (var key in this.deadlockConfig) {
  		if (config.deadlockConfig.hasOwnProperty(key)) {
  			this.deadlockConfig[key] = config.deadlockConfig[key]
  		}
  	}
  }
}
